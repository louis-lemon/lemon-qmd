/**
 * sdk-lemon.test.ts - Regression guard for the lemon-qmd app embedding path.
 *
...
beforeAll(async () => {
  root = await mkdtemp(join(tmpdir(), "qmd-sdk-lemon-"));
  wiki = join(root, "wiki");
...
describe("lemon-qmd SDK (app embedding path)", () => {
